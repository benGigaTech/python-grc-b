...
def get_pool():
    ...
    with _pool_lock:
        ...
        try:
            ...
            # If we're testing and the host is 'db', use 'db_test' instead
            if is_testing and db_host == 'db':
                db_host = 'db_test'
                logger.info(f"Using test database host: {db_host}")
            ...
            try:
                ...
                # If the connection fails and we're in testing mode, try with the Docker container name
                if is_testing and 'could not translate host name' in str(e):
                    container_name = f"python-grc-b-{db_host}-1"
                    logger.info(f"Trying with Docker container name: {container_name}")
                    _pool = ThreadedConnectionPool(
                        current_app.config['DB_POOL_MIN_CONN'],
                        current_app.config['DB_POOL_MAX_CONN'],
                        host=container_name,
                    ...
                    # Re-raise the exception if it's not a hostname issue or we're not in testing mode
                    raise
